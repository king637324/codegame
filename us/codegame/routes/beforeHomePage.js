...
router.get('/login', function(req, res, next) {
  req.logout()
  var local = res.locals.error //策略裡面的done
...
router.get('/register', function(req, res, next) {
  req.logout()
  res.render('beforeHome/register', {
    errors: ''
  });
});
// Post Sign Up
      ...
      if (!user) {
        User.getUserByEmail(email, function(err, user) {
          ...
          if (!user) {
            ...
            return res.json({
              responce: 'failRepeatEmail'
            });
          }
        })
      } else {
        return res.json({
          responce: 'failRepeatName'
        });
      }
    })
    // res.redirect('/login')
...
router.get('/forgetPass', function(req, res, next) {
  req.logout()
  res.render('beforeHome/forgetPass', {
    errors: ''
  });
})
  ...
  function(username, password, done) {
    User.findOne({
      ...
      User.comparePassword(password, user.password, function(err, isMatch) {
        if (err) throw err
        if (isMatch) {
          return done(null, user)   //產生req.user
        } else {
